...
import {fetchAcInfos, getAcInfo, getAcInfos, updateAcInfo} from "../slices/authSlice";
...
const AdminAcView: React.FC = () => {
    ...
    const handleAcStatusToggle = (roomNumber: string) => {
        ...
        const newAcStatus = !acInfo.acStatus;
        dispatch(updateAcInfo(acInfo.targetTemperature, newAcStatus, acInfo.acMode));
    }

    const handleModeChange = (roomNumber: string, e: SelectChangeEvent<string>) => {
        const acInfo = getAcInfoByRoomNumber(roomNumber);
        if (!acInfo) return; // 如果没有找到对应的信息，直接返回

        const newAcMode = e.target.value as string;
        dispatch(updateAcInfo(acInfo.targetTemperature, acInfo.acStatus, newAcMode));
    }
    ...
    const handleTemperatureChange = async (roomNumber: string, delta: number) => {
        ...
        await dispatch(updateAcInfo(newTargetTemperature, acInfo.acStatus, acInfo.acMode));
    }
    ...
    return (
        <>
            <NavigationBar/>
            <Container>
                <Typography variant="h4" align="center" gutterBottom style={{ paddingTop: "16px", paddingBottom: "16px"}}>
                    管理员空调信息
                </Typography>
                <Card elevation={3}>
                    <CardContent>
                                        ...
                                        <TableRow key={acInfo.roomNumber}>
                                            ...
                                            <TableCell>
                                                <ButtonGroup size="small" aria-label="small outlined button group">
                                                    <Button
                                                        onClick={() => handleTemperatureChange(acInfo.roomNumber, -1)}>-</Button>
                                            ...
                                            <TableCell>
                                                <Select
                                                    value={acInfo.acMode}
                                                    onChange={(e) => handleModeChange(acInfo.roomNumber, e)}
                                                    displayEmpty
                                                    size="small"
                                                >
                                            ...
                                            <TableCell>
                                                <Button
                                                    ...
                                                    onClick={() => handleAcStatusToggle(acInfo.roomNumber)}
                                                >
